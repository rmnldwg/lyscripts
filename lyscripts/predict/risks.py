...
:py:mod:`.predict.prevalences` script. Examples can be seen in an actual ``params.yaml``
file over in the `lynference`_ repository.
...
def predicted_risk(
    ...
    **_kwargs,
    ...
    """Compute the probability of arriving in a particular ``involvement``

    This is done given a particular ``t_stage`` and using a ``model`` with pretrained
    ``samples``. This probability can be computed for a ``given_diagnosis`` that was
    obtained using a modality with specificity & sensitivity provided via
    ``given_diagnosis_spsn``. If the model is an instance of ``MidlineBilateral``, one
    can specify whether or not the primary tumor has a ``midline_ext``.

    Both the ``involvement`` and the ``given_diagnosis`` should be dictionaries like
    this:
