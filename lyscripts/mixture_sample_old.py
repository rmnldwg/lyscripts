...
import pandas as pd
from lymixture.em import expectation, sample_fixed_mixture, sample_model_params
...
def _add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("-o", "--output", type=Path, help="Output path for samples")
    parser.add_argument(
        "-d", "--data", type=Path, required=True, help="Path to the data file."
    )
    parser.add_argument(
        "-c",
        "--continue_sampling",
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.mode == "fixed_mixture":
        backend, samples = sample_fixed_mixture(
            MIXTURE,
            steps=params["sampling"].get(
                "steps",
            ),
            filename=str(args.output) + "/fixed_mixture.hdf5",
            continue_sampling=args.continue_sampling,
        )
    elif args.mode == "fixed_latent":
        backend, samples = sample_model_params(
            MIXTURE,
            steps=params["sampling"].get("steps"),
            filename=str(args.output) + "/fixed_latent.hdf5",
            continue_sampling=args.continue_sampling,
        )
