...
def _add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--input", type=Path, help="File path of the mixture coefficients"
    )
...
def plot_2d_simplex(mixture_df, data, output, component_names=False):
    ...
    num_patients = dict(data["tumor", "1", "subsite"].value_counts())
    annotations = [f"{key}\n({num_patients[key]})" for key in mixture_df.columns]
    sizes = [num_patients[key] for key in mixture_df.columns]

    fig, bottom_ax = plt.subplots(figsize=(8, 2.5))
    bottom_ax.scatter(
        mixture_df.loc[0],
        np.zeros(len(mixture_df.loc[0])),
        s=sizes,
        c=colors_ordered,
        alpha=0.7,
        linewidths=0.0,
    ...
    x_positions = mixture_df.loc[0].values
    sorted_indices = np.argsort(x_positions)

    for i, idx in enumerate(sorted_indices):
        x = x_positions[idx]
        num = list(num_patients.values())[idx]
...
def plot_3d_simplex(mixture_df, data, output, component_names=False):
    ...
    extremes = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    for i in range(len(x_vals)):
        ax.scatter(
            x_vals[i], y_vals[i], s=sizes[i], color=colors_ordered[i], label=subsites[i]
        )
    ...
    for index in range(len(subsites)):
        legend_text.append(
            subsites[index] + ", " + str(odered_value_counts[index]) + " patients"
        )
    ...
    legend_elements = [
        ...
        for color, subsite in zip(colors_ordered, legend_text, strict=False)
    ]

    # Add a legend with fixed dot sizes
    ax.legend(
        handles=legend_elements, loc="upper right", title="Subsites", fontsize="small"
    )
    ...
    ax.plot(
        [extremes_x[-1], extremes_x[0]],
        [extremes_y[-1], extremes_y[0]],
        color="black",
        alpha=0.5,
    )
    ...
    midpoints_x = (
        (extremes_x[0] + extremes_x[1]) / 2,
        (extremes_x[1] + extremes_x[2]) / 2,
        (extremes_x[2] + extremes_x[0]) / 2,
    )
    midpoints_y = (
        (extremes_y[0] + extremes_y[1]) / 2,
        (extremes_y[1] + extremes_y[2]) / 2,
        (extremes_y[2] + extremes_y[0]) / 2,
    )

    # Draw lines from each vertex to the midpoint of the opposite side
    ax.plot(
        [extremes_x[0], midpoints_x[1]],
        [extremes_y[0], midpoints_y[1]],
        color="gray",
        linestyle="--",
        linewidth=1,
    )
    ax.plot(
        [extremes_x[1], midpoints_x[2]],
        [extremes_y[1], midpoints_y[2]],
        color="gray",
        linestyle="--",
        linewidth=1,
    )
    ax.plot(
        [extremes_x[2], midpoints_x[0]],
        [extremes_y[2], midpoints_y[0]],
        color="gray",
        linestyle="--",
        linewidth=1,
    )
    ...
    add_perpendicular_ticks(
        extremes_x[0], extremes_y[0], midpoints_x[1], midpoints_y[1]
    )
    add_perpendicular_ticks(
        extremes_x[1], extremes_y[1], midpoints_x[2], midpoints_y[2]
    )
    add_perpendicular_ticks(
        extremes_x[2], extremes_y[2], midpoints_x[0], midpoints_y[0]
    )
    ...
    if component_names:
        ...
        if "C32.0" in subsites:
            ...
            ax.text(
                scaled_extremes_x[component_larynx],
                scaled_extremes_y[component_larynx],
                "Larynx like",
                fontsize=10,
                ha="right",
                va="top",
        ...
        if "C01" in subsites:
            ...
            ax.text(
                scaled_extremes_x[component_oropharynx],
                scaled_extremes_y[component_oropharynx],
                "Oropharynx like",
                fontsize=10,
                ha="left",
                va="top",
        ...
        if "C03" in subsites:
            ...
            ax.text(
                scaled_extremes_x[component_oral_cavity],
                scaled_extremes_y[component_oral_cavity],
                "Oral cavity like",
                fontsize=10,
                ha="left",
                va="top",
        ...
        if "C13" in subsites:
            ...
            ax.text(
                scaled_extremes_x[component_hypopharynx],
                scaled_extremes_y[component_hypopharynx],
                "Hypopharynx like",
                fontsize=10,
                ha="left",
                va="top",
...
def main(args: argparse.Namespace):
    ...
    if nr_components == 2:
        plot_2d_simplex(mixture_df, data, output=args.output)
